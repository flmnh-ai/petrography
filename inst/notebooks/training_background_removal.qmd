...
format: html
params:
  data_dir: "../data/processed/background_removal"
...
config <- list(
  ...
  # Optional: keep remote data tidy on reruns
  rsync_mode = "mirror",
  dry_run = FALSE
)
...
# Prints a concise summary; aborts if invalid
validate_dataset(config$data_dir)

## size is included in validate_dataset()

cli_h2("Starting Training")

start_time <- Sys.time()

# Optional: quick HPC preflight if using HPC
if (!is.null(config$hpc_host)) {
  check_hpc(config$hpc_host, config$hpc_user)
}
